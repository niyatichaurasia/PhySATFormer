"""Standard channel-attention Transformer encoder block for the baseline architecture."""

import torch
from torch import nn

from src.models.standard_channel_attention import StandardChannelAttention


class StandardChannelAttentionBlock(nn.Module):
    """Pre-LayerNorm Transformer encoder block for channel-axis attention.

    This is the baseline counterpart of
    :class:`~src.models.channel_attention_block.ChannelAttentionBlock`.
    Instead of attending across the temporal axis, it applies
    :class:`~src.models.standard_channel_attention.
    StandardChannelAttention` across the *channel* axis of telemetry
    embeddings, with no physics-derived bias. Each sub-layer (attention
    and feed-forward) is wrapped in a residual connection and preceded
    by layer normalization (Pre-LN architecture).
    ...
    Attributes
    ----------
    attention : StandardChannelAttention
        Standard (non-physics-guided) multi-head channel attention
        module.
    ...
    >>> import torch
    >>> num_channels = 5
    >>> block = StandardChannelAttentionBlock(
    ...
    def __init__(
        ...
        self.attention = StandardChannelAttention(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply the Pre-LN standard channel attention block.
